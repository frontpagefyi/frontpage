...
const PublicEnv = z.object({
  ...
  NEXT_PUBLIC_FRONTPAGE_DID: z
    .string()
    .refine((val) => (isAtprotoDid as (val: string) => val is DID)(val), {
      message: "NEXT_PUBLIC_FRONTPAGE_DID must be a valid atproto DID",
    }),
});
...
export const publicConfig = PublicEnv.parse({
  ...
  NEXT_PUBLIC_FRONTPAGE_DID: process.env.NEXT_PUBLIC_FRONTPAGE_DID,
});
